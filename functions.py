...
def makro_splitter(product_name):
	...
	for unit in product_name:
		if not unit.isalpha() and not unit.isnumeric():
			if unit.isalnum():
				position = product_name.index(unit)

				try:
					if product_name[position + 1] == "x":
						next_container = product_name[position + 2]

						if next_container.isnumeric(): #300mm x 12
							temp_quantity = next_container

							for x in range(0, len(unit)):
								if ord(unit[x]) >= 97 and ord(unit[x]) <= 122:
									temp_units += unit[x]
								else:
									temp_size += unit[x]
							...
							temp_container = unit + "x" + next_container # 300mm x 400mm
							for x in range(0, len(possible_units)):
								if possible_units[x] in temp_container:
									temp_size = temp_container.replace(possible_units[x], "")
									temp_units = possible_units[x]
...
def mark_splitter(product_name):
	size = ""
	temp_size = ""
	units = ""
	temp_units = ""
	...
	for unit in name_container:
		if not unit.isnumeric() and not unit.isalpha():
			...
			# elif bracket in unit:
			# 	temp_quantity = unit.replace(bracket, "")
			# 	position = name_container.index(unit)

	  #       	size_container = name_container[position + 1]
	  #       	if size_container == "x":
	  #       		for x in range(0, len(units_container)):
	  #       			if ord(units_container[x]) >= 97 and ord(units_container[x]) <= 122:
	  #       				temp_units += units_container[x]
	  #       			else:
	  #       				temp_size += units_container[x]
	...
	return units, quantity, size, actual_product_name
